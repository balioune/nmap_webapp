...
def rmNotes(request, hashstr):
	...
	if re.match('^[a-f0-9]{32,32}$', hashstr) is not None:
		os.remove(settings.PROJECT_ROOT + '/../../notes/'+scanfilemd5+'_'+hashstr+'.notes')
...
def saveNotes(request):
	...
	if request.method == "POST":
		...
		if re.match('^[a-f0-9]{32,32}$', request.POST['hashstr']) is not None:
			f = open(settings.PROJECT_ROOT + '/../../notes/'+scanfilemd5+'_'+request.POST['hashstr']+'.notes', 'w')
...
def rmlabel(request, objtype, hashstr):
	...
	if re.match('^[a-f0-9]{32,32}$', hashstr) is not None:
		os.remove(settings.PROJECT_ROOT + '/../../notes/'+scanfilemd5+'_'+hashstr+'.'+objtype+'.label')
...
def label(request, objtype, label, hashstr):
	...
	if label in labels and objtype in types:
		if re.match('^[a-f0-9]{32,32}$', hashstr) is not None:
			f = open(settings.PROJECT_ROOT + '/../../notes/'+scanfilemd5+'_'+hashstr+'.'+objtype+'.label', 'w')
			f.write(label)
			f.close()
...
def port_details(request, address, portid):
	...
	oo = xmltodict.parse(open(settings.PROJECT_ROOT + '/../../xml/'+request.session['scanfile'], 'r').read())
		...
		for cpestr in cpe:
			...
			for host in cpe[cpestr]:
				...
				if type(cvejson) is list and len(cvejson) > 0:
					res[host] = cvejson[0]
					f = open(settings.PROJECT_ROOT + '/../../notes/'+scanfilemd5+'_'+hostmd5+'.cve', 'w')
...
def apiv1_hostdetails(request, scanfile, faddress=""):
	...
	oo = xmltodict.parse(open(settings.PROJECT_ROOT + '/../../xml/'+scanfile, 'r').read())
	...
	labelhost = {}
	labelfiles = os.listdir(settings.PROJECT_ROOT + '/../../notes/')
	for lf in labelfiles:
		m = re.match('^('+scanmd5+')_([a-z0-9]{32,32})\.host\.label$', lf)
		if m is not None:
			if m.group(1) not in labelhost:
				labelhost[m.group(1)] = {}
			labelhost[m.group(1)][m.group(2)] = open(settings.PROJECT_ROOT + '/../../notes/'+lf, 'r').read()

	# collect all notes in noteshost dict
	noteshost = {}
	notesfiles = os.listdir(settings.PROJECT_ROOT + '/../../notes/')
	for nf in notesfiles:
		m = re.match('^('+scanmd5+')_([a-z0-9]{32,32})\.notes$', nf)
		if m is not None:
			if m.group(1) not in noteshost:
				noteshost[m.group(1)] = {}
			noteshost[m.group(1)][m.group(2)] = open(settings.PROJECT_ROOT + '/../../notes/'+nf, 'r').read()
...
def apiv1_scan(request):
	...
	xmlfiles = os.listdir(settings.PROJECT_ROOT + '/../../xml/')
	...
	for i in xmlfiles:
		...
		try:
			...
			oo = xmltodict.parse(open(settings.PROJECT_ROOT + '/../../xml/'+i, 'r').read())
		except:
