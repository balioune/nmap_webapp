...
def token_check(token):
	tokenhash = open('/opt/afklm/scanning/opt/token/token.sha256').read().strip()
...
def get_cve(scanmd5):
	cvehost = {}
	cvefiles = os.listdir(settings.PROJECT_ROOT + '/../../notes/')
	for cf in cvefiles:
		...
		if m is not None:
			...
			if m.group(2) not in cvehost[m.group(1)]:
				cvehost[m.group(1)][m.group(2)] = open(settings.PROJECT_ROOT + '/../../notes/'+cf, 'r').read()
